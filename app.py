                ...
                if file is not None:
                    ...
                    if df is not None:
                        ...
                        if st.button(f"Run exponential growth fit for Species {i+1}", key=f"fit_button_{i}"):
                            ...
                            if mu is not None:
                                st.success(f"🎉 Fitting successful! Estimated mu: {mu:.5f}")
                                ...
                                st.session_state[f"species_{i}_data"]["use_log_scale"] = use_log_scale
                            else:
                        ...
                        fit_data = st.session_state.get(f"species_{i}_data", {})
                        fit_mu = fit_data.get("fit_mu", None)

                        if fit_mu is not None:
                            fit_start_plot = fit_data.get("fit_start", t_min)
                            fit_end_plot = fit_data.get("fit_end", t_max)
                            show_extrapolation = fit_data.get("show_extrapolation", False)
                            use_log_scale = fit_data.get("use_log_scale", False)

                            start_idx = (time >= fit_start_plot).idxmin() if fit_start_plot > t_min else 0
                            initial_value = avg_wells.iloc[start_idx]

                            plot_start = t_min if show_extrapolation else fit_start_plot
                            plot_end = t_max if show_extrapolation else fit_end_plot
                            ...
                            actual_times_fit = np.array(time[mask])
                            ...
                            actual = np.array(avg_wells[mask])
